...
# Enhanced XGBoost recommendations with discount features (135 features compatible)
...
print("🎯 Starting: ENHANCED XGBoost Discount + Temporal Features (135 features)")
...
# ———————————————————————
# 2. DISCOUNT TRACKING INITIALIZATION
# ———————————————————————
def initialize_discount_tracking(G):
    """Initialize discount tracking for all products"""
    for node, data in G.nodes(data=True):
        if data.get('label') == 'Product':
            if 'discount_events' not in data:
                data['discount_events'] = []  # List of (date, discount_value)

def record_product_discount(G, product_id, discount_date, discount_value):
    """Record when a product gets discounted"""
    if discount_value > 0.01:  # Only record meaningful discounts
        if 'discount_events' not in G.nodes[product_id]:
            G.nodes[product_id]['discount_events'] = []

        # Avoid duplicate dates
        existing_dates = [event[0] for event in G.nodes[product_id]['discount_events']]
        if discount_date.date() not in existing_dates:
            G.nodes[product_id]['discount_events'].append((discount_date.date(), discount_value))

# Initialize discount tracking
initialize_discount_tracking(G)
print("✅ Initialized discount tracking for products")
...
# ———————————————————————
# 5. DYNAMIC DISCOUNT ASSIGNMENT WITH TRACKING
# ———————————————————————
def add_dynamic_discounts_with_tracking(G, simulated_date):
    """Apply seasonal discounts and track them"""
    for node, data in G.nodes(data=True):
        if data.get('label') == 'Product':
            ...
            record_product_discount(G, node, simulated_date, discount)  # Track it!

# ———————————————————————
# 6. FEATURE EXTRACTION (135 features - COMPATIBLE WITH TRAINED MODEL)
# ———————————————————————
def get_link_features_with_discount_compatible(cust_id, prod_id, cust_df, prod_df, emb_dim, current_time):
    """Extract features compatible with trained model (135 features)"""
    try:
    ...
    # Build feature vector - EXACTLY 135 features (same as trained model):
    features = np.array([
        customer_degree, product_degree,           # 2
        *c_emb, *p_emb,                           # 128 (64 + 64)
        c_temp['recency'], c_temp['frequency'],   # 2 - Customer temporal
        p_temp['recency'], p_temp['frequency'],   # 2 - Product temporal
        current_discount                         # 1 - Current discount
    ])
...
# ———————————————————————
# 8. STOCHASTIC TEMPERATURE SOFTMAX SAMPLING (ENHANCED)
# ———————————————————————
def stochastic_temperature_softmax_sampling(probs, temperature=4.0, noise_scale=0.1):
    """
    Apply stochastic temperature softmax sampling for diverse recommendations

    Args:
        probs: Raw probabilities from model
        temperature: Controls exploration (higher = more random)
        noise_scale: Amount of noise to add for additional diversity

    Returns:
        sampling_probs: Probability distribution for sampling
    """
    # Add noise for additional diversity
    noise = np.random.normal(0, noise_scale, len(probs))
    noisy_probs = np.clip(probs + noise, 1e-10, 1 - 1e-10)

    # Convert to logits
    logits = np.log(noisy_probs / (1 - noisy_probs))
...
def get_recommendations_for_customer(G, customer_id, current_time, top_n=3, temperature=5.0):
    """Enhanced recommendation engine with stochastic temperature softmax sampling"""
    ...
    try:
        ...
        print(f"🔍 Probability range: {np.min(probs):.4f} to {np.max(probs):.4f}")
    except Exception as e:
        print(f"❌ Error in prediction: {e}")
        return []

    # Apply stochastic temperature softmax sampling
    sampling_probs = stochastic_temperature_softmax_sampling(probs, temperature=temperature, noise_scale=0.15)

    # Sample with category diversity (stochastic selection)
    seen_categories = set()
    selected = []
    attempts = 0
    max_attempts = top_n * 150  # More attempts for better exploration

    print(f"🔍 Sampling with temperature={temperature}, attempts={max_attempts}")

    while len(selected) < top_n and attempts < max_attempts:
        if len(product_ids) == 0 or sampling_probs.sum() == 0:
            break

        # Stochastic selection based on softmax probabilities
        try:
        ...
        # Category diversity enforcement (but allow some flexibility)
        if category in seen_categories:
            # Allow some repetition if we don't have enough variety
            if len(seen_categories) >= min(4, len(product_ids) // 4):
                attempts += 1
                continue
...
# ———————————————————————
# 11. RUN SIMULATION WITH FIXED DATES
# ———————————————————————
...
print("\n" + "="*80)
print("🎯 ENHANCED XGBOOST DISCOUNT + TEMPORAL RECOMMENDATIONS")
print("   → STOCHASTIC TEMPERATURE SOFTMAX SAMPLING")
print("   → Higher temperature (5.0) = More exploration")
print("   → Added noise = More varied results")
print("="*80)
...
for customer_id in sample_customers:
    ...
    for sim_date in simulation_dates:
        # Add dynamic discounts based on date (with tracking)
        add_dynamic_discounts_with_tracking(G, sim_date)
        month_name = sim_date.strftime('%b')
        print(f"\n📅 {sim_date.strftime('%Y-%m-%d')} (Month: {month_name})")

        # Get recommendations with enhanced stochastic softmax sampling
        recommendations = get_recommendations_for_customer(G, customer_id, sim_date, top_n=3, temperature=5.0)
...
print("\n" + "="*80)
print("🎉 ENHANCED XGBOOST DISCOUNT + TEMPORAL RECOMMENDATIONS COMPLETE")
print("="*80)
